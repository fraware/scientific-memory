...
- **2026-03**: Added `mapping.schema.json` for claim-to-declaration mapping (paper_id, namespace, claim_to_decl). No change to existing corpus; existing mapping.json files now validated in `validate-all`. Extended the unified gate runner behind `validate-all` to validate assumptions.json, symbols.json, and mapping.json per paper when present (SPEC Segment B).
