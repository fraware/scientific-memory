...
5. **Run benchmark:** `just benchmark`; the report includes `gold_claim_*`, `gold_assumption_*` when applicable, and source-span alignment fields on `tasks.gold` when any gold `source_spans.json` exists. CI runs the benchmark on every PR. **Current state:** all six formalized core papers have gold (claims and `source_spans.json` where claims carry spans; assumptions copied when present in corpus). The two hard-dimension stress scaffolds are indexed but not yet gold-populated. Add gold for each new non-scaffold paper before merging (see [paper-intake.md](paper-intake.md) and [ROADMAP.md](../ROADMAP.md)).
...
- **Proof completion:** Reads all manifests' coverage_metrics; outputs proof_completion_rate and Milestone 3 progress. Aggregate machine-checked declaration totals are authoritative in [status/repo-snapshot.md](status/repo-snapshot.md) (regenerate with `just repo-snapshot`).
