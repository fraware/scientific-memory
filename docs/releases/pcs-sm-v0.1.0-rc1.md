...
1. Signed-bundle byte parity vs `pcs-core` (via sync check)
2. `tests/pcs/test_canonical_rc.py` and `tests/pcs/test_pcs_release_negatives.py`
3. `just pcs-import-rc-bundle` and `just pcs-render-claim claim-pcs-qc-release-v0.1`
4. Portal read-model zod contract
