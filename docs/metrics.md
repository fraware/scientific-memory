...
| LLM eval regression anchors (non-canonical) | Task `llm_eval`: reviewed `benchmarks/llm_eval/<paper_id>/reference_llm_*_proposals.json` vs gold claim ids, corpus `mapping.json`, and static Lean conversion-ready checks. Declared prompt template digests appear at top level of the benchmark report as `llm_prompt_templates` (id → SHA-256). See [benchmarks/llm_eval/README.md](../benchmarks/llm_eval/README.md) and [ADR 0013](adr/0013-llm-evaluation-policy.md). |
